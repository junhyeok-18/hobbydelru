	...
	var imgDir = "/resources/js/images/";			// directory for images ... e.g. var imgDir="/img/"
    ...
    */
	...
	if (dom){
		...
		document.write ("<div onclick='bShow=true' id='calendar' style='z-index:+999;position:absolute;visibility:hidden;'><table cellspacing='0' width="+((showWeekNumber==1)?250:150)+" style='height: 300px; font-family:cnsa_Light; font-size: 18px; border: 1px solid #BFBFBF; border-spacing: 0px;'><tr style='background-color: #FFF; height: 50px;'><td><table width='"+((showWeekNumber==1)?248:240)+"'><tr><td style='font-family:cnsa_Light; font-size:15px; text-align:center;'><font color='#000000'><B><span id='caption'></span></B></font></td><td><a href='javascript:hideCalendar()'><IMG SRC='"+imgDir+"close.png' WIDTH='15' HEIGHT='13' BORDER='0' ALT='Close the Calendar'></a></td></tr></table></td></tr><tr><td bgcolor=#ffffff><span id='content'></span></td></tr>");
			
		if (showToday==1){
			document.write ("<tr bgcolor=#F3EFEC><td style='padding:5px' align=center><span id='lblToday'></span></td></tr>");
		}
	
		document.write ("</table></div><div id='selectYear' style='z-index:+999;position:absolute;visibility:hidden;left:620px;top:290px;margin-left: 3px;margin-top:10px;'></div><div id='selectMonth' style='z-index:+999;position:absolute;visibility:hidden;margin-top:10px; width: 50px; left:680px;'></div>");
	}
	...
	function init()	{
		if (!ns4){
			...
			sHTML1="<span id='spanYear' style='border-width:0;cursor:pointer' onmouseover='swapImage(\"changeYear\",\"drop2.gif\");window.status=\""+selectYearMessage+"\"' onmouseout='swapImage(\"changeYear\",\"drop1.png\");window.status=\"\"' onclick='popUpYear()'></span>&nbsp;";
...
//			sHTML1+="<span id='spanLeft'	style='border-width:0;cursor:pointer' onmouseover='swapImage(\"changeLeft\",\"left2.gif\");window.status=\""+scrollLeftMessage+"\"' onclick='javascript:decMonth()' onmouseout='clearInterval(intervalID1);swapImage(\"changeLeft\",\"left1.gif\");window.status=\"\"' onmousedown='clearTimeout(timeoutID1);timeoutID1=setTimeout(\"StartDecMonth()\",500)' onmouseup='clearTimeout(timeoutID1);clearInterval(intervalID1)'>&nbsp<IMG id='changeLeft' SRC='"+imgDir+"left1.gif' width=10 height=11 BORDER=0>&nbsp</span>&nbsp;";
			...
			sHTML1+="<span id='spanMonth' style='border-width:0;cursor:pointer;margin-left:20px;'	onmouseover='swapImage(\"changeMonth\",\"drop2.gif\");window.status=\""+selectMonthMessage+"\"' onmouseout='swapImage(\"changeMonth\",\"drop1.png\");window.status=\"\"' onclick='popUpMonth()'></span>&nbsp;";
			// link to next month 
...
//		var	sTmp;
	...
	function constructMonth() {
		...
		if (!monthConstructed) {
			...
			document.getElementById("selectMonth").innerHTML = "<table width=50	style='font-family:cnsa_Light; margin-left: 66px; font-size:15px; border-width:1; border-style:solid;' bgcolor='white' cellspacing=0 onmouseover='clearTimeout(timeoutID1)'	onmouseout='clearTimeout(timeoutID1);timeoutID1=setTimeout(\"popDownMonth()\",100);event.cancelBubble=true'>" +	sHTML +	"</table>";
	...
	function constructYear() {
		...
		if (!yearConstructed) {
			...
			document.getElementById("selectYear").innerHTML	= "<table width=44 style='font-family:cnsa_Light; font-size:15px; border-width:1; border-style:solid;'	bgcolor='white' onmouseover='clearTimeout(timeoutID2)' onmouseout='clearTimeout(timeoutID2);timeoutID2=setTimeout(\"popDownYear()\",100)' cellspacing=0>"	+ sHTML	+ "</table>";
   ...
   function WeekNbr(n) {
	...
	function constructCalendar() {
		...
		sHTML =	"<table style='font-family:cnsa_Light; margin: auto; font-size:15px; border-spacing: 0px; width: 246px;'><tr style='background-color: #F7F7F7; text-align:center; height: 38px;'>";
		...
		for	(var i=0; i<7; i++)	{
			if(dayName[i]=="일"){
				sHTML += "<td style='padding: 10px 0px 10px 0px;' ><B><font color='red'>"+ dayName[i]+"</font></B></td>";
			}else if(dayName[i]=="토"){
				sHTML += "<td style='padding: 10px 0px 10px 0px;' ><B><font color='#0054FF'>"+ dayName[i]+"</font></B></td>";
			}else{
				sHTML += "<td style='padding: 10px 0px 10px 0px;' ><B>"+ dayName[i]+"</B></td>";
			}
		}
		sHTML +="</tr><tr style='text-align:center;'>";
		
		if (showWeekNumber==1){
			sHTML += "<td style='padding: 10px 0px 10px 0px;' >" + WeekNbr(startDate) + "&nbsp;</td>";
		}

		for	( var i=1; i<=dayPointer;i++ ){
			sHTML += "<td style='padding: 10px 0px 10px 0px;' >&nbsp;</td>";
		}
	
		for	( var datePointer=1; datePointer<=numDaysInMonth; datePointer++ ){
			dayPointer++;
			sHTML += "<td style='padding: 10px 0px 10px 0px;' >";
			sStyle=styleAnchor;
			...
			if(dayPointer % 7 == (startAt * -1)+1){
				...
				sHTML += "<a "+dateMessage+" title=\"" + sHint + "\" style='"+sStyle+"' href='javascript:dateSelected="+datePointer + ";javascript:weekend=1;closeCalendar();'>&nbsp;<font color="+ color +">" + datePointer + "</font>&nbsp;</a>";
				...
				sHTML += "<a "+dateMessage+" title=\"" + sHint + "\" style='"+sStyle+"' href='javascript:dateSelected="+datePointer + ";javascript:weekend=0;closeCalendar();'>&nbsp;<font color="+ color +">" + datePointer + "&nbsp;</a>"; 
			}
			...
			if ((dayPointer+startAt) % 7 == startAt) { 
				...
				if ((showWeekNumber==1)&&(datePointer<numDaysInMonth)){
					sHTML += "<td style='padding: 10px 0px 10px 0px;' >" + (WeekNbr(new Date(yearSelected,monthSelected,datePointer+1))) + "&nbsp;</td>";
				}
			}
		}
